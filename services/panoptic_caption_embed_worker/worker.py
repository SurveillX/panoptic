...
def _process_message(engine, r, msg, worker_id: str, embedding_client) -> bool:
    """
    ...
    with engine.connect() as conn:
        with LeaseHeartbeat(engine, job_id, worker_id) as hb:
            ...
            try:
                job_state = run_caption_embed_job(
                    conn,
...
def run_worker(engine, r, worker_id: str, embedding_client) -> None:
    """
    Main worker loop: block-reads and processes caption_embed jobs indefinitely.
    """
    stream = STREAM_FOR_JOB_TYPE["caption_embed"]
    group = GROUP_FOR_JOB_TYPE["caption_embed"]
    ...
    while True:
        ...
        try:
            _process_message(engine, r, msg, worker_id, embedding_client)
        except Exception as exc:
            log.exception(
...
def main() -> None:
    ...
    ensure_image_caption_collection(vector_size=len(probe_vector))
    log.info(
        "Qdrant ready: %s collection=%s dim=%d",
        QDRANT_URL, "image_caption_vectors", len(probe_vector),
    )
